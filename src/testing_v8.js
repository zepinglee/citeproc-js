        ...
        for (var i=0,ilen=name.length;i<ilen;i++) {
            ...
            if (this.input[filename]){
                ret.push(this.input[filename]);
                   ...
                   print("Huh?");
                var datastring = read("data/" + filename + ".txt", "UTF-8");
                eval( "obj = " + datastring );
...
RhinoTest.prototype._fixAllNames = function(input){
    for (var i=0,ilen=input.length;i<ilen;i++) {
        ...
        for (var j=0,jlen=CSL.CREATORS.length;j<jlen;j++) {
            var key = CSL.CREATORS[j];
            if (obj[key]){
                for (var k=0,klen=obj[key].length;k<klen;k++) {
                    var entry = obj[key][k];
...
RhinoTest.prototype.retrieveLocale = function(lang){
    if ("undefined" === typeof CSL_JSON) {
        var ret = read("./locale/locales-"+lang+".xml", "UTF-8");
        ret = ret.replace(/\s*<\?[^>]*\?>\s*\n/g, "");
    } else {
        var s = read("./locale/locales-"+lang+".json");
        ret = JSON.parse(s);
    }
    return ret;
};
