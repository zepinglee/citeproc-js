 ...
 */
CSL.Engine.prototype.makeBibKey = function(item){
	...
	for each (var n in CSL.CREATORS){
		if (Item[n]){
			var myname = Item[n];
			if (myname && myname.family){
				myname = myname.family.replace(/\s+/,"");
			} else if (myname && myname.literal){
				myname = myname.literal;
			...
			if (!myname){
				myname = this.getTerm("references","short",0);
			}
		}
	}
	var year = "MYYEAR";
	var suffix = "";
	if (this.registry.registry[item] && this.registry.registry[item].disambig[2]){
		var num = parseInt(this.registry.registry[Item.id].disambig[2], 10);
		suffix = this.fun.suffixator.format(num);
	}
	return (myname + year + suffix);
	// XXXXX: Do something here!
};

CSL.Engine.prototype.getBibKey = function(item){
